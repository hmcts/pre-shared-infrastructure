...
resource "azurerm_virtual_machine_extension" "data_gateway_configure" {
  ...
  protected_settings = jsonencode({
    "commandToExecute" : "powershell -ExecutionPolicy unrestricted -NoProfile -NonInteractive -command \"cp c:/azuredata/customdata.bin c:/azuredata/datagateway-init.ps1; .\\datagateway-init.ps1\"",
    "scriptVariables" : {
