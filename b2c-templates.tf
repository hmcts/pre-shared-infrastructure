locals {
  ...
  b2c_file_details = {
    ...
    b2c_file_path => {
      ...
      content = contains(local.content_file, split(".", b2c_file_path)) ? replace(replace(file("${path.module}/${b2c_file_path}"), "{env}", var.env), "{hostname}", local.hostname) : ""
  ...
  hostname = var.env == "production" ? "portal.pre-recorded-evidence.justice.gov.uk" : "pre-portal.${local.env_long_name}.platform.hmcts.net"
  containers = [{
