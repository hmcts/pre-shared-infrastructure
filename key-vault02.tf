...
module "key-vault02" {
  ...
  name                    = "${var.product}-02-${var.env}" 
  product                 = var.product
  env                     = var.env
  tenant_id               = data.azurerm_client_config.current.tenant_id
  object_id               = var.jenkins_AAD_objectId
  resource_group_name     = razurerm_resource_group.rg02.name
...
resource "azurerm_key_vault_access_policy" "power_app_access02" {
...
#####################################
resource "azurerm_key_vault_access_policy" "dts_pre_access02" {
  key_vault_id            = module.key-vault02.key_vault_id
  # application_id        = var.app_id
...
#####################################
resource "azurerm_key_vault_access_policy" "dts_cft_developers_access02" {
  key_vault_id            = module.key-vault02.key_vault_id
  # application_id        = var.app_id
...
#    DTS PRE Admin
#####################################
resource "azurerm_key_vault_access_policy" "dts_dts_pre_project_admin_access02" {
  key_vault_id            = module.key-vault02.key_vault_id
  # application_id        = var.app_id
...
resource "azurerm_key_vault_access_policy" "devops_access02" {
  key_vault_id            = module.key-vault02.key_vault_id
  # application_id        = var.app_id
...
resource "random_string" "vm_username02" {
  count   = var.num_vid_edit_vms
  length  = 4
  special = false
}

resource "random_password" "vm_password02" {
  count            = var.num_vid_edit_vms
  length           = 16
  special          = true
  override_special = "#$%&@()_[]{}<>:?"
...
resource "azurerm_key_vault_secret" "vm_username_secret02" {
  count        = var.num_vid_edit_vms
...
resource "azurerm_key_vault_secret" "vm_password_secret02" {
  count        = var.num_vid_edit_vms
...
#     private_connection_resource_id = module.key-vault02.key_vault_id
