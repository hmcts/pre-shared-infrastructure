...
resource "azuread_service_principal" "resource_api_sp" {
  client_id = data.azuread_application.pre_apim_b2c_app.client_id
}
...
resource "azuread_application_api_access" "client_needs_api" {
  ...
  api_client_id  = azuread_service_principal.resource_api_sp.client_id
  scope_ids      = [
    azuread_service_principal.resource_api_sp.oauth2_permission_scope_ids["api.request.b2c"]
  ]
}
