...
module "finalsa_storage_account02" {
  source                   = "git@github.com:hmcts/cnp-module-storage-account?ref=master"
  env                      = var.env
  storage_account_name     = replace("${var.product}finalsa02${var.env}", "-", "")
  resource_group_name      = azurerm_resource_group.rg02
  location                 = azurerm_resource_group.rg.location #As recommended by MS
  account_kind             = "StorageV2"
  account_tier             = var.sa_account_tier
  account_replication_type = var.sa_replication_type
  ...
  depends_on = [ module.key-vault02]
}

module "ingestsa_storage_account02" {
  source                          = "git@github.com:hmcts/cnp-module-storage-account?ref=master"
  env                             = var.env
  storage_account_name            = replace("${var.product}ingestsa02${var.env}", "-", "")
  resource_group_name             = azurerm_resource_group.rg02
  location                        = azurerm_resource_group.rg.location #As recommended by MS azurerm_resource_group.rg.location
  account_kind                    = "StorageV2"
  account_tier                    = var.sa_account_tier
  account_replication_type        = var.sa_replication_type
...
module "ingestsa02_storage_account02" {
  source                          = "git@github.com:hmcts/cnp-module-storage-account?ref=master"
  env                             = var.env
  storage_account_name            = replace("${var.product}ingestsa02${var.env}", "-", "")
  resource_group_name             = azurerm_resource_group.rg02
...
resource "azurerm_key_vault_secret" "sa_storage_account02_connection_string" {
  name         = "sa-storage-account-connection-string"
  value        = module.sa_storage_account02.storageaccount_primary_connection_string
  key_vault_id = module.key-vault02.key_vault_id
}
resource "azurerm_key_vault_secret" "finalsa_storage_account02_connection_string" {
  name         = "finalsa-storage-account-connection-string"
  value        = module.finalsa_storage_account02.storageaccount_primary_connection_string
  key_vault_id = module.key-vault02.key_vault_id
}

resource "azurerm_key_vault_secret" "ingestsa_storage_account02_connection_string" {
  name         = "ingestsa-storage-account-connection-string"
  value        = module.ingestsa_storage_account02.storageaccount_primary_connection_string
  key_vault_id = module.key-vault02.key_vault_id
}
